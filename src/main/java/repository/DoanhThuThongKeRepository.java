...
public class DoanhThuThongKeRepository {
    ...
    public List<Object[]> getDTTKNgay(LocalDateTime ngayDau, LocalDateTime ngayCuoi) {
        List<Object[]> listSP = new ArrayList<>();

        try {
            Session session = HibernateUtil.getFACTORY().openSession();
            Query query = session.createQuery("""
                                              select trunc(hd.ngayThanhToan) , sum(hd.tongTien - hd.tienGiam) 
                                              from HoaDon hd
                                              where hd.ngayThanhToan between :ngayDau and :ngayCuoi
                                              group by trunc(hd.ngayThanhToan)
                                               """);
